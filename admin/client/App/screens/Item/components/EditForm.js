...
var EditForm = React.createClass({
	...
	preview (e) {
		e.target.href = `/${this.props.data.fields.slug}.html`;
	},
	renderKeyOrId () {
	...
	renderFooterBar () {
		...
		const previewingButtonText = loading ? 'Saving' : 'Preview';
		...
		return (
			<FooterBar style={styles.footerbar}>
				<div style={styles.footerbarInner}>
					...
					{this.props.list.id === 'pages' && (
						<a onClick={this.preview} href="#" target="_blank" className="base_b0alru-o_O-base_xhel8e" style={{marginLeft: '1em', display: 'inline-block'}}>{previewingButtonText}</a>
					)}
					{!this.props.list.noedit && (
